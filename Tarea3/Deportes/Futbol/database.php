<?php
	...
	class Database{
		...
		public function create($nombres,$apellidos,$telefono,$direccion,$correo_electronico){
			$sql = "INSERT INTO `futbolistas` (nombre, apellido, carrera, posicion, email) VALUES ('$nombres', '$apellidos', '$telefono', '$direccion', '$correo_electronico')";
		...
		public function update($nombres,$apellidos,$telefono,$direccion,$correo_electronico, $id){
			$sql = "UPDATE futbolistas SET nombres='$nombres', apellidos='$apellidos', telefono='$telefono', direccion='$direccion', correo_electronico='$correo_electronico' WHERE id=$id";
			$stmt = $this->con->prepare($sql);
  			$stmt->execute();
			if($stmt){
				return true;
